...
class ImageUtils:
    ...
    @staticmethod
    def project_point(context, point: Tuple[int, int] or Event, invert: bool = False, return_relative: bool = False, round_to_int: bool = True) -> Tuple[int, int]:
        if invert:
            reg_point = context.region.view2d.view_to_region(*point)
            if return_relative:
                region = context.region
                relative_reg_point = (reg_point[0]/region.width, reg_point[1]/region.height)
                if round_to_int:
                    return (int(relative_reg_point), int(relative_reg_point))
                else:
                    relative_reg_point
            return reg_point
        if isinstance(point, Event):
            reg_point = (point.mouse_region_x, point.mouse_region_y)
        relative_view_point = context.region.view2d.region_to_view(*point)
        if return_relative:
            return relative_view_point
        return ImageUtils.absolute_point(context, relative_view_point)
    ...
    def absolute_point(context, point: Tuple[int, int], clip: bool = False) -> Tuple[int, int]:
        ...
        return (point[0]*img.size[0], point[1]*img.size[1])

    @staticmethod
    def relative_point(context, point: Tuple[int, int], clip: bool = False) -> Tuple[int, int]:
        img = context.space_data.image
        if clip:
            point = (clamp(point[0], 0, img.size[0]), clamp(point[1], 0, img.size[1]))
        return (point[0]/img.size[0], point[1]/img.size[1])
