...
class PixelPaintWidget(Gizmo):
    ...
    def test_select(self, ctx, mp):
        ...
        '''
        cur_pos = Vector(mp)
        origin = Vector(ImageUtils.project_point(ctx, (0, 0), invert=True, clip=False))
        ...
        pixel_size = self.pixel_size # reg_image_size.x * self.ratio * zoom
        ...
        '''
        origin = ImageUtils.project_to_region(ctx, (0, 0), clip=False, round_to_int=False)
        pixel_size = self.pixel_size
        fixed_mp = (
            origin[0] + (mp[0]-origin[0]) * pixel_size,
            origin[1] + (mp[1]-origin[1]) * pixel_size
        )
    ...
    def update_pixel_size(self, ctx):
        ...
        '''
        p_view_origin = Vector(ImageUtils.project_point(ctx, (0, 0), clip=False)) #, return_relative=True)) #, invert=True)
        p_view_origin_plus = Vector(ImageUtils.project_point(ctx, (1, 0), clip=False))#, return_relative=True)) #, invert=True)
        p_view_max_x = Vector(ImageUtils.project_point(ctx, (image_size.x, 0), clip=False))#, return_relative=True))
        ...
        reg_xmax = ImageUtils.project_to_region(ctx, (view_width, 0),  clip=False)[0]
        reg_y0   = ImageUtils.project_to_region(ctx, (0, 0),           clip=False)[1]
        reg_ymax = ImageUtils.project_to_region(ctx, (0, view_height), clip=False)[0]

        img_region_width  = (reg_xmax - reg_x0)
        img_region_height = (reg_ymax - reg_y0)
        pixel_size = (reg_x1 - reg_x0)
        pixel_size /= img_region_width
